async function loadGallery() {
  ...
  try {
    ...
    formations.forEach(item => {
      ...
      card.innerHTML = `
        <img src="../${encodeURI(item.image)}" alt="${item.title}" loading="lazy" class="max-w-full h-auto mb-3 border rounded" style="background:${item.themeColor};">
        <h2 class="text-xl font-semibold mb-1">${item.title}</h2>
        <p class="mb-2 text-gray-700">${item.description}</p>
        <a href="./formation.html?id=${item.id}" class="mt-auto px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">View Full</a>
      `;
